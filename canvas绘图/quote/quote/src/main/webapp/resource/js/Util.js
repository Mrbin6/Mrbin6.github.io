...
//��ȡdiv������
function getDivParam(id){
	var divParam = document.getElementById(id);
	var width = divParam.offsetWidth;
	var height = divParam.offsetHeight;
	var top = divParam.offsetTop;
	var left = divParam.offsetLeft;
	if(left == 0){
		left = 1;
	}
	return {
...
function hasClass( elements,cName ){    
	return !!elements.className.match( new RegExp( "(\\s|^)" + cName + "(\\s|$)") );//(\s|^)cName(\s|$)
};
